...
- **A fork's `main` follows `samouraiworld/main` automatically.** `.github/workflows/sync-main-from-upstream.yml` fast-forwards it hourly and on demand, and fails loudly rather than force-pushing when the two diverge. A red run on that workflow means the local sync check below will fail too: fix the divergence before starting any task.
